 ...
 * return: an array of strings with the name of cards
 */
...
void write_card(FILE* card, const char * const message) {
	...
	while(character = message[i++]) {
		...
		// This condition is written to only 125 characters
		if((word_count % 125) == 0) {
			fputc('\n', card);
		}
	}
...
/**
 * This function gets a file's size.
 * parameters: a pointer to file
 ...
 * parameters: none
 * return: a random character generated
 */
char generate_random_character(void) {
